...
{
    QString rawVendor = proofApp->settings()->mainGroup()->value("vendor_id", "0x0b67", Proof::Settings::NotFoundPolicy::Add).toString().trimmed();
    QString rawProduct = proofApp->settings()->mainGroup()->value("product_id", "0x555e", Proof::Settings::NotFoundPolicy::Add).toString().trimmed();
...
{
    sendAnswer(socket, QJsonDocument(stateToJson(m_handler->instantState())).toJson(QJsonDocument::Compact), "application/json", 200, "OK");
}
...
void WeighingScaleRestServer::rest_get_Weight_Stable(QTcpSocket *socket, const QStringList &, const QStringList &, const QUrlQuery &queryParams, const QByteArray &)
{
    ...
    if (immediate) {
        sendAnswer(socket, QJsonDocument(stateToJson(m_handler->lastStableState())).toJson(QJsonDocument::Compact), "application/json", 200, "OK");
    } else {
...
QMap<QString, QPair<QDateTime, QVariant>> WeighingScaleRestServer::healthStatus(bool) const
{
    QDateTime current = QDateTime::currentDateTimeUtc();
    return QMap<QString, QPair<QDateTime, QVariant> > {
        {QStringLiteral("device_vendor_id"), {current, QString("0x%1").arg(m_handler->vendorId(), 4, 16, QChar('0'))}},
        {QStringLiteral("device_product_id"), {current, QString("0x%1").arg(m_handler->productId(), 4, 16, QChar('0'))}},
        {QStringLiteral("device_alive"), {current, m_handler->isAlive()}},
        {QStringLiteral("msecs_since_last_read_from_device"), {current, m_handler->elapsedSinceLastMessage()}}
    };
}
